            ...
            if 'agent' in st.session_state:
                ...
                with col1:
                    if st.button("🔍 检查所有文件格式", type="secondary", key="check_batch"):
                        check_batch_files_format(uploaded_files, temp_file_paths)
...
def display_batch_file_info(uploaded_files):
    ...
    with st.expander("📋 查看文件列表", expanded=True):
        for i, file in enumerate(uploaded_files, 1):
            ...
            st.write(f"{i}. **{file.name}** - {file_size:,} 字节 ({file_size/1024:.1f} KB)")
...
def check_file_format(temp_file_path, file_name):
    ...
    with st.spinner("检查文件格式..."):
        file_info = st.session_state.agent.chat(f"检查文件格式，文件名：{file_name}，文件路径：{temp_file_path}")
        st.info(file_info)

def test_file_upload(temp_file_path, file_name):
    """测试文件上传"""
    with st.spinner("测试文件上传..."):
        test_result = st.session_state.agent.chat(f"测试文件上传，文件名：{file_name}，文件路径：{temp_file_path}")
...
def start_single_diagnosis(temp_file_path, file_name):
    ...
    with st.spinner("正在进行故障诊断...这可能需要一些时间..."):
        ...
        # 然后在主区域以优化格式显示
        display_diagnosis_report(diagnosis_result, "📊 单文件诊断结果报告")
...
def check_batch_files_format(uploaded_files, temp_file_paths):
    ...
    with st.spinner("检查所有文件格式..."):
        for i, (uploaded_file, temp_path) in enumerate(zip(uploaded_files, temp_file_paths)):
            ...
            file_info = st.session_state.agent.chat(f"检查文件格式，文件名：{uploaded_file.name}，文件路径：{temp_path}")
            st.info(file_info)
...
def start_batch_diagnosis(uploaded_files, temp_file_paths):
    ...
    with st.spinner(f"正在进行批量诊断...处理 {len(uploaded_files)} 个文件，请耐心等待..."):
        ...
        diagnosis_result = st.session_state.agent.chat(f"批量诊断这些文件：{file_info_str}")
        
        # 添加到消息历史
        file_names = [f.name for f in uploaded_files]
        add_to_chat_history("user", f"批量诊断 {len(uploaded_files)} 个文件：{', '.join(file_names)}")
        add_to_chat_history("assistant", diagnosis_result)
        ...
        # 然后在主区域以优化格式显示
        display_diagnosis_report(diagnosis_result, "📊 批量诊断结果报告")
